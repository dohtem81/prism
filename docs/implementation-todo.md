...
| P1-20 | Cross-cutting | Security | Add rate limiting, quotas, and abuse protections for message send volume and API usage | DONE | Medium | 2026-08-30 | Redis-backed fixed-window limits (fail-open on Redis errors) now cover message send (per-user/per-room), room creation, room membership admin actions, and websocket connections per user; 429s include Retry-After/X-RateLimit-* headers. See docs/rate-limiting-and-quotas-plan.md. |
